    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch(
          `https://api.paylix.gg/v1/groups?timestamp=${Date.now()}`,
          {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${process.env.NEXT_PUBLIC_PAYLIX_API_KEY}`,
            },
          }
        );
      ...
      <div className="container relative mx-auto px-4">
        ...
        <RevealAnimation>
          <div className="w-full">
            <DisplayProducts products={productsData} error={error} />
          </div>
        </RevealAnimation>
