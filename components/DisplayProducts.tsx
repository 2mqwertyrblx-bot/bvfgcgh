...
type ProductGroup = {
  title: string;
  products_bound: PaylixProduct[];
};

const DisplayProducts = ({ products, error }: { products: any; error: string | null }) => {
  const handleScriptLoad = () => {
    console.log("Paylix script loaded successfully");
  };

  const getProductFeatures = (product: PaylixProduct) => {
    let features: string[] = [];
  ...
  const setProductsInAscendingOrder = (products: PaylixProduct[]) => {
    return products.sort((a, b) => a.price - b.price);
  };

  if (error) {
    return <div className="px-4 mt-10 text-center text-red-400">Error loading products: {error}</div>;
  }
  ...
  const productGroups = products?.data?.groups || products?.groups;
                        ...
                        <div className="px-6 pt-4 pb-6">
                          ...
                          <div className="relative group/button overflow-hidden rounded-lg border border-purple-500/20 hover:border-purple-500/30 mt-6">
                            ...
                            <button
                              ...
                              data-paylix-product={product?.uniqid}
                              type="button"
                            >
